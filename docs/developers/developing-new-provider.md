...
To publish a provider so that it can be downloaded and added via the [`cloudquery init [provider]`](/docs/cli/commands/cloudquery_init.md) command you'll need to [raise an issue](https://github.com/cloudquery/cloudquery/issues) with the relevant details so that we can add it to our registry.
