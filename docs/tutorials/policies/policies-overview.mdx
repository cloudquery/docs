...
CloudQuery allows you to use SQL to check various conditions about your cloud deployment. 
...
You can get a `.json` report instead if you use the `--output-dir` flag, or, you can get the policy results written to your postgreSQL database with `--enable-db-persistence`.
You can find the schema for the postgreSQL policy results [here](https://docs.cloudquery.io/docs/cli/policy/storage).
...
your `cloudquery.yml` file. This means that before running policies, you must run `cloudquery fetch` to populate this
