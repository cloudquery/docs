...
`AWS_PROFILE` environment variabler, or similar.
...
The roles in your target account will need to have permissions to read your cloud configuation.
...
CloudQuery to fetch. 
[See also this blogpost](https://alestic.com/2015/10/aws-iam-readonly-too-permissive/).
:::
...
Next, attach the `AllowAssumeRoleIntoTargetAccounts` policy to the user CloudQuery will run as (replace `<SourceAccountUserName>` and `<SourceAcocountId>`):
...
Your `config.hcl` file should look similar to the following (call `cloudquery init aws` if you don't have one yet).
...
`accounts` is a confusing name - the block should really be named `account`. Each `accounts` represents a **single** acount
