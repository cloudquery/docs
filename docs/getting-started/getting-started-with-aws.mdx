...
After installing CloudQuery, you need to generate a `config.yml` file that will describe which cloud provider you want to use and which resources you want CloudQuery to ETL:
...
or, configure your desired profile in the `local_profile` field of your CloudQuery `config.yml`:
...
Once `config.yml` is generated and you are authenticated with AWS, run the following command to fetch the resources.
